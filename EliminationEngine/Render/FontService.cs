...
namespace EliminationEngine.Render
{
    public class FontService
    {
		...
		public static Bitmap? RenderString(string text, Face face)
        {
			//GC.Collect(); // for memory leaks
			...
			using (var g = Graphics.FromImage(bmp))
			{
				...
				for (int i = 0; i < text.Length; i++)
				{
					...
					if ((ftbmp.Width > 0 && ftbmp.Rows > 0))
					{
						...
						Bitmap cBmp = ftbmp.ToGdipBitmap(Color.Transparent);
						rc.Width = cBmp.Width;
